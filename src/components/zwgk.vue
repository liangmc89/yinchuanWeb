<template>
  <div class="content-layout">
    <!--<q-window-resize-observable @resize="onResize" />-->
      ...
      <swiper :options="zwgkwiperOption" ref="content-navs-swiper" class="fit">
        <swiper-slide :key="index" v-for="(item,index) in lanmudata.dataList1" style="width: auto">
          <div @click="zwgkNavBtnClick(index,item)" class="fit" >
            <div class="zwgk-navbtn fit" :style="{backgroundImage:'url('+getUrl(item.Icon)+')'}">
              <div class="text-no-wrap text-center">{{item.Title}}</div>
            </div>
          </div>
        </swiper-slide>
      </swiper>
